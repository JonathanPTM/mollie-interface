<?php
/*
 *  mollie_interface - PaymentChargebackHandler.php Copyright (c) 2023 PTMDevelopment -  Jonathan. All rights reserved.
...
class PaymentChargebackHandler implements Handler
{
     ...
     */
    public function __construct(Payment $molliePayment, \PTM\MollieInterface\models\Payment$record)
    {
    ...
    public function execute()
    {
        ...
        {
            $this->record->update([
                'amount_charged_back' => $this->molliePayment->amountChargedBack->value
            ]);
            Event::dispatch(new ReceivedChargeback($this->record, ($this->molliePayment->amountChargedBack->value - $localAmount)));
        }
    }
}