<?php
/*
 *  mollie_interface - PaymentRefundHandler.php Copyright (c) 2023 PTMDevelopment -  Jonathan. All rights reserved.
...
class PaymentRefundHandler implements Handler
{
    ...
    public function execute(PaymentProcessor$interface)
    {
        ...
        {
            $this->record->update([
                'amount_refunded' => $this->molliePayment->amountRefunded->value
            ]);
            Event::dispatch(new ReceivedChargeback($this->record, $this->molliePayment->amountRefunded->value));
        }
    }
}