...
class FirstPaymentSubscriptionBuilder implements SubscriptionBuilder
{
    ...
    {
        $subscription = $this->owner->subscriptions()->where('subscribed_on', $this->thread)->where('plan_id', $this->plan->id ?? 0)->first();
        if (!$subscription) {
        ...
        $this->redirectUrl = route('ptm_mollie.redirect.subscription', ['id' => $subscription->id]);
